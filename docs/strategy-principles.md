...
#### 2026-09-23 SaaS 도메인 적용 재검토 — 보류 유지 (남헌 위임)

SaaS 피봇 코퍼스에 이 고도화를 적용할지 프로덕션 DB 로 재검토했다. **결론: 보류 유지.** 이유는 셋이고 전부 실측이다.

1. **등급항이 지금 상수다.** 승인 무브 36건(SaaS 11건 포함)이 **전부 등급 A** 이고 B·C·D 는 0건이다(2026-09-23 DB 실측). `GRADE_RANK['A'] × 10 = 30` 이 모든 카드에 같게 붙으므로, SP-024 가 지목한 역전(낱말 1개 A등급 31점 > 낱말 5개 C등급 15점)은 이 코퍼스에서 **구조적으로 발생하지 않는다.** 순위는 사실상 겹친 낱말 수로만 갈린다 — SP-024 가 원했던 상태와 같다.
2. **1순위 정렬축이 바뀌었다.** 2026-09-23 `KIND_MISMATCH_MODE='bonus'` 전환으로 `KIND_MATCH_BONUS = 100` 이 들어갔다(`lib/cases/advisor.ts`). 등급(최대 30)·낱말 수 둘 다 압도하므로 "등급이 겹침 강도를 압도한다"는 SP-024 의 문제 제기는 이제 **제품 종류 다음의 3순위 문제**다.
