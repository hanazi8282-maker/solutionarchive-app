...
읽는 사람은 **"만들 줄은 아는데 그걸 돈으로 바꾸는 법을 모르는 사람"**이다. 그래서 글의
목적은 브랜드 사례 소개가 아니라 **그 사람이 자기 상황에 옮겨 쓸 수 있는 인사이트**다.

- 무브의 `transfer_note`(독자가 내일 할 수 있는 최소 행동 1개)가 이 글의 뼈대다.
  그게 본문에 남지 않았으면 브랜드 이야기만 쓴 것이다.
- `preconditions` 가 적혀 있으면 숨기지 마라. "이건 이런 조건에서만 된다"를 빼면
  독자가 따라 하다 넘어진다.
- 등급은 **근거 표기에 쓰는 값**이지 소재 선정의 기준이 아니다. 앞에 내세우지 않는다.
- ⛔ 등급 D 무브로 쓰는 글에는 **숫자를 넣지 마라.** D 는 수치가 없는 무브라 그 숫자는
  우리 근거에서 나온 게 아니다. 발행 게이트 CG-2 가 스테이징을 막는다(exit 4).
