...
4. **큐 관리** — 무엇을 왜 조사할지 `research_queue` 에 미리 정한다.
   소재 비율(1인·초기 창업가 6.5 : 대형 3.5, `content/guides/케이스-작성-가이드.md` §9)은 여기서
   라운드마다 점진적으로 맞춘다 — 2026-09-15 기준 적립 33건 중 초기 소재 3~4건이라 신규 대상은
   1인·초기 창업가 우선, 대형은 독자가 옮길 부분이 분명할 때만.
5. **분석 칼럼** — 적립 케이스를 `content/guides/케이스-작성-가이드.md` 기준의 칼럼으로 옮긴다
   (2026-09-15 남헌 재설계). 스레드는 칼럼에서 인사이트 1개씩 뗀다.
...
### 칼럼 (건당) — 2026-09-15 남헌 확정 파이프라인
- `drafts/columns/<날짜>-<slug>.md`(본문 + 본문 밖 `## 근거 메모`) + `.research.md`(출처·11항목·§10 점검).
- 3,000~8,000자, 필수 5요소 전부. **사고의 흐름과 적용은 각 1건 이상, 당사자 1인칭 출처에서.**
- **독립 검증은 필수 단계다.** 쓴 에이전트가 아닌 에이전트가 근거 URL 을 전부 열어 문장 단위로
  대조하고 최소 수정한 뒤 `.verify.md`(판정·걸린 문장·원문 인용·수정문·§10 재점검)를 남긴다.
  작가의 자체 점검은 검증 근거가 아니다 — 1차 배치 5편에서 작가 전원 "예"였는데 검증에서
  편당 7~12건이 걸렸다. `.verify.md` 없는 칼럼은 남헌 검토 대상이 아니다.
...
- 마이그레이션 적용이 필요한 상황 — **승인 없이 적용하지 않는다.** 파일을 만들고 올린다.
  남헌 승인이 내려오면 그때 적용한다 (`CLAUDE.md §10.2`).
