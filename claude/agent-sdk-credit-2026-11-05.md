...
## ⚠️ 2026-09-26 정정
- 이 대장의 "크레딧 경로" 열은 **$250 과 무관**하다. $250 은 클라우드 세션 전용 런칭 크레딧(11/4 PT 만료)이고 `claude -p`/Actions 는 거기서 빠지지 않는다. 위 표의 작업들은 OAuth 토큰 계정의 구독 한도를 쓴다. 상세: reports/2026-09-25/agent-sdk-credit-plan.md 말미 정정 절.
- "$2.46" 은 budget.ts 자체 추정치다. 실측(total_cost_usd)은 2026-09-26 부터 claude-cli 로그에 남는다.
- #4 T2 판정: **전환 안 됨**(워크플로에 env 두 줄 누락). 남헌 재결정 전까지 Gemini 유지.
