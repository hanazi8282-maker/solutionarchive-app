...
export async function POST(req: Request) {
  ...
  // 단 api_tokens 를 못 읽은 것(DB 장애)은 "토큰 없음"이 아니다 → 503 (tokenFailure).
  const tokenResult = await loadThreadsToken()
  if (tokenResult.status === 'unavailable') {
    const f = tokenFailure(tokenResult)
    return NextResponse.json(f.body, { status: f.status })
  }
  if (tokenResult.status !== 'ok') return NextResponse.json({ ok: true, skipped: 'threads 토큰 없음' })
  const { accessToken: token, userId } = tokenResult.creds
