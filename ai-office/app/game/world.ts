...
/** 부서 방 배치 — 열 위치는 고정, 행은 부서 수에 맞춰 늘어난다 */
...
const ROW_Y = Array.from(
  { length: Math.ceil(DEPARTMENTS.length / COL_X.length) },
  (_, row) => ROW_TOP + row * ROW_PITCH,
);

/** 방 하나에 들어가는 책상 수 — 폭에서 계산한다 (책상 3칸 + 통로 1칸) */
const DESK_W = 3;
const DESK_PITCH = DESK_W + 1;
const INNER_W = DEPT_W - 2; // 좌우 벽 제외
const DESK_COUNT = Math.floor((INNER_W - 1) / DESK_PITCH);
/** 좌석의 방 기준 x 오프셋 */
const DESK_DX = Array.from({ length: DESK_COUNT }, (_, i) => DESK_W + i * DESK_PITCH);

// 부서가 늘면 방이 바닥 밖으로 나가 캐릭터가 벽에 갇힌다 — 조용히 깨지지 않게 여기서 막는다.
const lastRowBottom = ROW_Y[ROW_Y.length - 1] + DEPT_H;
if (lastRowBottom > ROWS - 1) {
  const fits = COL_X.length * Math.floor((ROWS - 1 - ROW_TOP - DEPT_H) / ROW_PITCH + 1);
  throw new Error(
    `부서 ${DEPARTMENTS.length}개는 사무실(${COLS}×${ROWS})에 안 들어갑니다. ` +
      `company.config.ts 의 DEPARTMENTS 를 ${fits}개 이하로 줄이거나 app/game/world.ts 의 ROWS 를 늘리세요.`,
  );
}

// 좌석보다 사람이 많으면 남는 인원의 자리가 입구 복도로 밀려난다(조용한 실패) — 미리 막는다.
for (const dept of DEPARTMENTS) {
  const headcount = STAFF_LIST.filter((s) => s.dept === dept.id).length;
  if (headcount > DESK_COUNT) {
    throw new Error(
      `${dept.name}(${dept.id}) 인원이 ${headcount}명인데 방에는 좌석이 ${DESK_COUNT}석뿐입니다. ` +
        `company.config.ts 의 STAFF_LIST 에서 이 팀을 ${DESK_COUNT}명 이하로 줄이거나 app/game/world.ts 의 DEPT_W 를 늘리세요.`,
    );
  }
}
...
function deptRoom(index: number): Room {
  ...
  const desks: Desk[] = DESK_DX.map((dx) => ({
    deskX: x + dx - 1,
    deskY: y + 5,
    seat: { x: x + dx, y: y + 6 },
  }));
...
for (const room of DEPT_ROOMS) {
  for (const desk of room.desks) {
    PROPS.push({ kind: "desk", x: desk.deskX, y: desk.deskY, w: DESK_W, h: 1 });
  }
...
PROPS.push({ kind: "screen", x: 28, y: 3, w: 5, h: 1, label: "오늘 안건" });
