...
### 11.5 🔴 부트스트랩 절차가 코드에 없었다 — 거부 로그에 발신자 id 가 없다 (2026-09-02)

§11 "사람이 할 일" 2번은 이렇게 적혀 있다 — *"값은 첫 공유 때 서버 로그에
찍히는 `userRequest.user.id` 다. 먼저 한 번 보내서 로그로 확인한 뒤 넣는다."*

**그 로그가 없었다.** 라우트는 거부 시 사유만 남겼다:

```ts
console.warn(`[kakao-webhook] rejected: ${gate.reason}`)
```

`gate.reason` 은 `KAKAO_ALLOWED_USER_IDS 미설정 — 잠긴 상태다` /
`userRequest.user.id 없음` / `허용목록에 없는 발신자` 셋뿐이고 **id 를 담지
않는다.** 발화 전문을 찍는 `console.log` 는 게이트를 **통과한 뒤에** 있다.

실측 증거 — 카카오 egress 가 뚫린 뒤 실제로 도달한 첫 요청(08:42:31 UTC):

```
[kakao-webhook] rejected: KAKAO_ALLOWED_USER_IDS 미설정 — 잠긴 상태다
```

id 가 없다. **막힌 이유는 보이는데 푸는 열쇠가 안 보인다.**

#### 왜 이게 조용한 교착인가

허용목록은 fail-closed 다(§11 보안). 비어 있으면 전부 막는다 — 의도된
설계다. 그런데 **거기 넣을 값을 알아내는 유일한 경로가 거부 로그**다.
그 로그에 id 가 없으면 순환이 닫힌다:

- 사람은 공유한다 → "등록되지 않은 발신자입니다"
- 로그를 본다 → 사유만 있다
- 다시 공유한다 → 같은 결과

에러가 나지 않는다. 워크플로도 안 죽는다. 문서에는 절차가 적혀 있고,
코드는 그 절차를 수행하지 않는다. §7.1 이 말하는 형태 — **"확인 불가"가
"정상 동작"처럼 보인다.**

#### 고쳤다

`lib/insight/capture.ts` 에 `rejectionLogLine(reason, userId)` 를 두고
라우트가 그것을 쓴다.

```
[kakao-webhook] rejected: KAKAO_ALLOWED_USER_IDS 미설정 — 잠긴 상태다 · user.id=abc123
```

- **응답에는 절대 싣지 않는다.** 허용목록의 존재와 동작 방식을 외부에
  노출한다. id 는 서버 로그에만 남는다
- id 가 없거나 공백이면 `(없음)` 으로 명시한다. `undefined` 를 그대로
  흘리면 "값이 없다"와 "키가 없다"가 구분되지 않는다
- **모든 거부에 남긴다.** botUserKey 는 채널 범위의 불투명 값이라 그 자체로
  신원을 드러내지 않고, 남이 이 URL 을 찾아 두드릴 때도 같은 줄이 그것을
  드러낸다
- 셀프테스트 59 → **66건**(id 보존 / 사유 병기 / 태그 / `(없음)` 2경우 /
  `undefined` 누출 금지 / 한 줄 보장)

#### 순서

이 수정이 프로덕션에 올라간 **뒤에** 첫 공유를 해야 id 를 얻는다. 그전에
공유하면 로그에 또 사유만 남는다.
