...
독자는 **"만들 줄은 아는데 그걸 돈으로 바꾸는 법을 모르는 사람"**이다. 창업·제작에서
실제로 막혀 있는 사람이고, 브랜드 소개를 읽으러 오지 않았다.

"이 브랜드 소개"가 아니다. **그 독자가 자기 상황에 옮겨 쓸 수 있는 실행(무브) 1개 이상**을
근거와 함께 꺼내는 것이다. 옮길 수 없는 사실은 아무리 흥미로워도 적지 않는다.

**목표는 등급 A 가 아니라 옮길 수 있는 무브 1개다.** 등급은 그 무브에 붙는 성질이지
조사의 목표가 아니다.

## 반드시 지킬 것 — 순서대로

- **독자 축을 먼저 채운다.** `reader_problem`(이 이야기를 옮겨 쓸 독자가 지금 무엇에
  막혀 있나) → `transfer_note`(그 독자가 **내일** 할 수 있는 최소 행동 1개) →
  `preconditions`(옮기려면 뭐가 있어야 하나). 어휘 정본은 `config/reader-problems.json`
  이고 코드에 박혀 있지 않다 — 맞는 코드가 없으면 억지로 끼워 맞추지 말고 조사 노트에 적는다.
  `transfer_note` 가 비면 독자가 가져갈 게 없는 케이스다.
- **이식성을 스스로 판정하지 않는다.** `transferability`(HIGH/MEDIUM/LOW)는 사람이
  승인할 때 고른다. 초안 JSON 에 그 필드를 쓰지 않는다.
...
항목당 한 줄. slug / **독자 문제(reader_problem)** / **각 무브의 transfer_note 한 줄** /
병목 / 무브 수 / 각 무브의 등급과 그 근거 / validate exit 코드 / 못 찾은 것.
독자 축이 앞에 온다 — 등급부터 보고하면 읽는 사람의 순서도 등급부터가 된다.
