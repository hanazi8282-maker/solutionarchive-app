...
function parseAnswers() {
  ...
  for (let i = 0; i < marks.length; i++) {
    ...
    //
    // ★ 정규식(`/\*\*([^*]{2,24})\*\*/g`)으로 잡으면 안 된다. 길이 상한을 넘는
    // 굵은 구절은 통째로 건너뛰는 게 아니라 **짝이 어긋난다** — 그 구절의 닫는
    // `**` 가 다음 구절의 여는 `**` 와 짝지어져, 원래 굵게 표시된 적 없는
    // 사이 텍스트("-추가:" 같은 것)가 키워드로 들어왔다. 그러면 피평가자가
    // 맞혀야 할 이유가 없는 문자열이 채점 키워드가 된다.
    // `split('**')` 의 홀수 인덱스는 여는/닫는 순서를 그대로 따르므로 짝이 안 깨진다.
    const bolds = block.split('**')
      .filter((_, i) => i % 2 === 1)
      .map(s => s.trim())
