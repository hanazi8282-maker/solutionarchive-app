...
**조사 대상은 SaaS·소프트웨어 사업이다 — 1인·소규모 팀이 만든 것을 먼저 고른다** (2026-09-24).
큐가 시장을 명시적으로 다르게 지정하지 않는 한 소비재 브랜드를 고르지 않는다. WebSearch 는
소비재 쪽으로 쏠리니 의식적으로 되돌려라. 대형 SaaS(Notion·Slack·Stripe 급)를 맡았으면
**`1인 창업가가 옮길 수 있는 부분`이 무엇인지를 `transfer_note` 에 반드시 명시한다** — 그게 없으면
그 케이스는 독자에게 "큰 회사 얘기"로만 남는다.
...
- **`transfer_note` 가 비면 validate 가 error 로 막는다**(2026-09-24부터). 경고가 아니다 — 파일이 통과하지 않는다.
...
- **사고의 흐름을 1건 이상, 당사자 1인칭 출처에서 꺼낸다** (2026-09-15 남헌). 창업자 인터뷰·블로그·
  팟캐스트·실적발표 콜을 **최소 1건** 직접 열고, 어떤 관찰에서 어떤 추론으로 결정에 갔는지를 무브의
  `claim` 또는 조사 노트에 적는다. 공시·언론만으로 조사를 끝내지 않는다. `transfer_note` 와 이 항목이
  각 1건 이상 없으면 "완료"가 아니라 "조사 미완"으로 보고한다.
...
- 소재 비율(1인·초기 창업가 6.5 : 대형 3.5)은 조사원이 아니라 큐를 채우는 오케스트레이터가 라운드마다
  점진적으로 맞춘다 — 조사원은 맡은 브랜드가 대형이면 "독자가 옮길 부분"을 반드시 명시한다.
