-- 롤백: theqoo · todayhumor 커뮤니티 소스 kill-switch
...
UPDATE public.review_sources
   SET enabled = false, disabled_reason = '롤백: 커뮤니티 소스 round-2(theqoo·todayhumor) 철회'
 WHERE key IN ('theqoo', 'todayhumor');

-- 행 자체를 지우려면 review_targets / review_fingerprints / analysis_inputs 의
-- source_key IN ('theqoo','todayhumor') 행을 먼저 정리해야 한다(FK).
-- 그건 데이터 삭제라 사람이 판단한다.
--
-- product_ref 컬럼 주석은 되돌리지 않는다 — 주석은 동작에 영향이 없고,
-- 되돌리면 SSRF 경고와 SP-026 경고까지 함께 사라진다.
