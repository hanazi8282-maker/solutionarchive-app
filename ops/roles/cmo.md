...
## 콘텐츠 판단 위계 — 헌법 (남헌 결정 2026-09-14)

초안의 문체·구성 판단이 부딪히면 번호가 작은 쪽이 이긴다. 작가 문서·명령·
`learned-patterns.md` 는 이 절을 가리킬 뿐 다시 적지 않는다.

1. **반응률** — 발행 성과로 검증된 패턴이 최상위다. 어떤 패턴이든 최종 채택·강등·기각은
   반응률(`lib/insight/patterns.ts` `decide()`)로만 판정한다. 발행자 수정 패턴(`edit-`)도 면제 없다.
2. **발행자(남헌) 수정본** — 반응률 근거가 없거나 부족할 때의 기준. `edit-` 패턴과 발행자 문체 규칙.
3. **작가(AI) 기본값** — 위 둘과 부딪히면 진다.
   `references/threads-playbook.md`(레퍼런스 역산 T-01~T-16)가 여기 들어간다.
   그 문서 §3-0 의 실측은 **남의 계정** 숫자라 1층이 아니다 — 우리 발행 반응률만 1층이다.
   미해결 에스컬레이션 4건(E-1~E-4)은 그 문서 §12 에 있다.

흐름: `edit-` 패턴을 초안에 먼저 반영 → 발행 → 반응률이 판결한다. 그러려면 초안에 적용한
패턴 키를 `stage.json` 의 `applied_patterns` 에 반드시 남긴다(없으면 `[]`). 이 목록이 없으면
그 글의 반응률이 패턴 판정으로 돌아오지 않는다.
