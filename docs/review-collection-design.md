...
## §13.19 403 을 쿼터 소진과 차단으로 가른다 (2026-09-02)

아키텍처 검토(§B)가 지목한 🔴 위험을 실제 소스를 붙이기 **전에** 고쳤다.
YouTube·네이버 키가 들어오는 순간 바로 터지는 문제였다.

### 무엇이 문제였나

`lib/review/health.ts` 는 403/429 를 **무조건 차단**으로 읽고 `broken` +
`enabled=false` 를 내렸다. 그런데 공식 API 는 **일일 한도를 다 쓰면 403 을
준다.**

즉 정상적인 쿼터 소진이 이렇게 기록된다:

```
disabled_reason: 차단 응답 403 — 재시도하지 않고 소스를 중단한다
```

다음날 아침 사람은 **차단당한 줄 안다.** 실제로는 어제 몫을 다 쓴 것뿐이고
오늘 다시 돌면 정상이다. 상태 코드만으로 성공/실패를 판정하지 말라는
§7.1 이 **실패 쪽에서 재발하는** 형태다 — 지금까지 잡아온 사례는 전부
"실패를 성공으로" 읽는 방향이었는데 이건 반대다.

### 어떻게 갈랐나

`classifyBlockedResponse(body, quotaMarkers)` 를 순수 함수로 넣었다.

**판단할 수 없으면 차단이다.** 표지 목록이 없거나·본문이 비었거나·표지가
안 보이면 전부 `blocked`. 두 오판의 비용이 대칭이 아니라서 그렇다:

- 차단을 쿼터로 오인 → 차단당한 소스를 계속 두드린다. **영구 차단에 가까워진다**
- 쿼터를 차단으로 오인 → 소스가 하루 꺼진다. 되살리는 건 UPDATE 한 줄이다

표지는 **어댑터가 선언한다**(`ReviewSourceAdapter.quotaMarkers`). DB 컬럼이
아니라 어댑터에 둔 이유는, 이 수정이 **프로덕션 DDL 없이 지금 들어갈 수
있어야** 하기 때문이다. §B 의 `review_sources` 확장 마이그레이션은 승인
대기 중이고, 그걸 기다리는 동안 이 위험을 열어둘 이유가 없다.

다나와는 쿼터 개념이 없으므로 선언하지 않는다 → **기존 동작 완전 무변경.**

### 판정 순서가 바뀌었다

```
1. 차단 응답(403/429)        → broken + 중단
2. 파싱 성공률 < 8/10        → broken + 중단
3. 쿼터 소진                 → ok (중단 안 함, 연속 0건도 안 올림)   ← 신규
4. 연속 신규 0건 3회 이상    → degraded
5. 그 외                     → ok
```

**3이 2보다 아래인 이유**: 러너가 403 을 받으면 파싱 전에 끊으므로 쿼터
응답 본문은 `parseFailures` 에 안 섞인다. 즉 파싱 지표는 그날 실제로 받아온
페이지들의 독립적인 신호이고, **파서가 진짜로 깨졌다면 쿼터가 소진된 날에도
그 사실이 먼저 드러나야 한다.**

### 🔴 가장 놓치기 쉬운 경계 — 연속 0건 카운터

쿼터가 소진되면 그날 신규가 0건일 수 있다. 그걸 세면 **사흘 뒤**
`연속 3회 신규 0건 — 증분이 끝났거나 조용히 막혔을 수 있다` 가 뜬다.
**원인은 쿼터인데 증분 종료로 진단된다.**

그래서 쿼터 소진은 카운터를 올리지 않는다. 차단을 신규 0건과 다른 사건으로
다루던 기존 판단과 같은 이유다.

### 같이 고친 것 — §13.17 의 미해결 2건

**① dry-run 의 `신규 0건` 은 0 이 아니라 측정 안 함이다.**
`runner.ts` 가 `if (opts.dryRun) continue` 를 `newCount++` 앞에서 하므로
구조상 항상 0이다. 이제 `신규 —(dry-run 은 판정하지 않음)` 으로 찍는다.
요약 줄과 타깃별 줄 **양쪽** 다 고쳤다.

**② `진행` 이 상한 도달을 안 드러냈다.**
`outcome` 초기값이 `'진행'` 이라, 20페이지 루프가 break 없이 완주하면 그대로
남는다 = 상한에 걸려 잘렸다는 뜻인데 이름은 "정상 진행"으로 읽힌다.
`페이지 상한 20 도달 — 다음 실행에서 이어 읽는다` 로 바꿨다(§7.2).

로컬 dry-run 실측:

```
- 파싱 294건(실패 0) · 신규 —(dry-run 은 판정하지 않음) · 폴백키 0건 · robots 회피 0건
### 타깃별
- `18753650` — 페이지 상한 20 도달 — 다음 실행에서 이어 읽는다 · 신규 —(…)
- `14071175` — 끝까지 읽음 · 신규 —(…)
```

### 검증

- `review-health-selftest` 40 → **60건** (분류기 10경계 + 판정 10경계)
- `review-runner-selftest` 73 → **82건** — 러너가 실제로 그 분류를 쓰는지
  **경계면을 따로** 테스트했다. 분류기 단위 통과를 통합 근거로 쓰지 않는다(§7.1)
- 표지 미선언 어댑터(다나와)는 쿼터 본문이 와도 차단으로 센다 — 회귀 없음 확인
- tsc 통과, 로컬 dry-run 실측

---

## §13.20 원문 폐기 배치 — 설계 §9 를 실제로 집행한다 (2026-09-02)

§13.14 검토에서 드러난 것: **`purged_at` 컬럼도, 인덱스도, CHECK 제약도 전부
있는데 그걸 실행하는 코드가 리포에 없었다.** `scripts/` 에 purge 파일 0건,
워크플로 호출 0건, `purged_at` 을 쓰는 코드는 검증기뿐. **§9 설계가 스키마만
있고 동작이 없는 상태**였고, 수집이 시작되면 `raw_text` 가 무기한 쌓인다.

수집 데이터가 거의 없는 지금이 만들 타이밍이다 — 쌓인 뒤에 만들면 이미
들어온 원문의 폐기 시점을 소급해야 한다.

### 스키마가 이미 정해 둔 계약

- `source_key IS NULL` = 사람이 붙여넣은 원문 → **절대 지우지 않는다.**
  다시 구할 수 없다
- `source_key IS NOT NULL` = 수집기가 넣은 것 → `collected_at + 30d` 후 폐기
- `analysis_inputs_purge_trace_check` = `raw_text IS NOT NULL OR purged_at IS
  NOT NULL` → **`raw_text` 와 `purged_at` 을 같은 UPDATE 에서 써야 한다.**
  raw_text 만 비우면 제약 위반으로 실패한다. 그 제약이 있는 이유가 "아직 안
  받은 것"과 "받았다가 폐기한 것"을 구분하기 위해서다
- `analysis_inputs_purgeable_idx` = 배치가 훑는 경로

### 만든 것

- `lib/review/purge.ts` — 판정 순수 함수(`decidePurge`/`planPurge`/
  `purgePatch`/`purgeLine`). 되돌릴 수 없는 동작이라 경계를 코드가 아니라
  테스트가 붙잡는다
- `scripts/review-purge.mjs` — **기본이 dry-run.** 실제로 지우려면 `--apply`
- `scripts/review-purge-selftest.mjs` — 29건
- `nightly-review-collect.yml` 에 스텝 추가. **새 크론을 만들지 않았다**

### 판정 불가는 지우지 않되, 조용히 넘기지도 않는다

§7.1 의 "확인 불가를 양성으로 접지 마라"가 여기서는 **방향이 반대**다.
다른 검사에서는 판정 불가를 실패로 보고하라는 뜻이었는데, 여기서 안전한 쪽은
**"안 지운다"** 이다. 둘 다 지키면 "안 지우되 보고한다"가 된다.

판정 불가로 떨어지는 경우:

- `source_key` 는 있는데 `collected_at` 이 없다 → 나이를 알 수 없다.
  **수집기가 `collected_at` 을 안 넣는 경로가 생겼다는 뜻**이고, 그러면 그
  행들은 영영 폐기되지 않는다. 경보로 올린다
- `collected_at` 을 날짜로 못 읽는다 / 미래 시각이다

⚠️ 다만 **사람 원문(`source_key IS NULL`)은 `collected_at` 이 없어도 '보존'
이지 '판정 불가'가 아니다.** 판정 불가로 떨어뜨리면 매일 밤 경보가 뜨는데
그 행은 애초에 폐기 대상이 아니라 조치할 것이 없다. 늘 뜨는 경보는 안 읽는
경보가 된다.

### 🔴 종료코드 127 — 성공 실행이 실패로 보고되던 경로

로컬 실측에서 잡았다. dry-run 이 정상 종료했는데 **종료코드가 127** 이었다.
...
`process.exit()` 를 Supabase 클라이언트가 핸들을 열어 둔 상태에서 부르면
libuv 가 어서션으로 죽는다. **CI 잡의 성공/실패가 이 값에 걸려 있으므로,
정상 실행이 실패로 보고된다.** `process.exitCode` 만 세우고 이벤트 루프가
스스로 비도록 바꿨다 → 0 확인.

이건 §7.1 의 거울상이다. 지금까지는 "실패인데 성공으로 보인다"를 잡아왔는데
이건 "성공인데 실패로 보인다"이고, 방치하면 매일 밤 붉은 잡을 보다가
결국 무시하게 된다.

### 실제로 지우는 것은 스위치를 켜야 한다

스케줄 실행은 **기본이 dry-run** 이다. 되돌릴 수 없는 동작을 무인 상태에서
바로 열지 않는다.

- 수동 실행: `purge_apply` 입력을 켠다
- 자동 실행: 리포 변수 `REVIEW_PURGE_APPLY=true` 를 설정한다.
  **코드 변경 없이 대시보드에서 켠다** — 며칠 dry-run 으로 대상 건수를 눈으로
  본 뒤 켜는 것을 권한다

`if: always()` 로 붙였다. 수집이 실패해도 보존 기간은 집행돼야 한다 —
수집 실패를 이유로 폐기가 밀리면 지우기로 한 원문이 남는다.

### 30일의 근거를 용량으로 적지 않는다

§13.14 에서 확인했듯 현 스코프에서 원문+지문 전량이 4MB, 무료 티어의 0.8%다.
**용량 압박은 실재하지 않는다.** 폐기 근거를 용량으로만 적어 두면 나중에
"안 지워도 되네"로 뒤집힌다. 실질 근거는 **데이터 위생 — 남의 리뷰 원문을
무기한 보관하지 않는다** 이다.

### 실측

```
## 원문 폐기 (dry-run — 지우지 않음)
- 후보 0건 · 폐기 대상 0건 · 보존 0건 · 판정 불가 0건
- 보존 기간 30일 · 상한 1000건
```

DB 대조: `analysis_inputs` 10행 전부 `source_key IS NULL`(사람 원문)이라
후보에서 제외되는 것이 맞다. 종료코드 0.

---

## §13.21 크론을 정각에서 옮겼다 (2026-09-02)

§13.18 이 기록한 미발화의 후속. 두 야간 워크플로가 **정각**에 걸려 있었다.

```
Nightly Review Collect  0 18 * * *  →  37 17 * * *   (KST 03:00 → 02:37)
Nightly Insight Loop    0 19 * * *  →  41 18 * * *   (KST 04:00 → 03:41)
```

정각·30분은 전 세계 크론이 몰리는 시각이라 Actions 가 스케줄 실행을 뒤로
미루거나 통째로 건너뛴다. 실측 근거: 2026-09-01 18:00 UTC 에 예정된 수집
워크플로의 첫 스케줄 실행이 **1시간이 지나도 발화하지 않았고**, 그 시점까지
이 워크플로의 `schedule` 실행 이력은 **0건**이었다(수동 실행만 있었다).
워크플로 상태는 둘 다 `active` 였다.

순서(수집이 인사이트보다 한 시간 앞)는 유지했다. 인사이트 루프가
`post_performance` 를 읽을 때 수집기가 방금 돈 상태인 게 낫다.

**이게 원인이라는 것은 아직 확정이 아니다.** 스케줄 지연은 다른 이유로도
생긴다. 다음 밤에 두 워크플로가 실제로 발화하는지 보면 판정된다 —
발화하면 정각 혼잡이 원인이었던 것이고, 또 안 뜨면 다른 데를 봐야 한다.

### 검사 방법도 고쳤다

워크플로를 고친 뒤 "YAML 파싱 ok"를 정규식 매칭으로 확인할 뻔했다.
**§7.1 사례 4번("파싱 ok 가 실은 탭 문자만 셌다")을 그대로 반복하는
형태다.** `js-yaml` 로 실제 파싱해 `cron` / `steps` / `inputs` /
`permissions` 를 객체로 꺼내 확인했다.

```
nightly-review-collect: cron=["37 17 * * *"] steps=[…,"Purge expired raw text"]
                        inputs=["dry_run","source","targets","purge_apply"]
                        permissions={"contents":"read"}
nightly-insight-loop:   cron=["41 18 * * *"] permissions={"contents":"write"}
```

---

## §13.22 App Store 고객 리뷰 어댑터 (2026-09-02)
...
### 실측으로 확정한 계약 (2026-09-02)

**① 종료 신호는 `feed.entry` 의 부재다.** 마지막 페이지 다음은 HTTP 200 인데
`entry` 키가 아예 없다(1,515B).

**`link[rel=last]` 는 쓰지 않는다.** 실측에서 entry 35건이 있는 1페이지가
**자기를 `last` 라고 하면서 동시에 `next` 로 2페이지를 가리켰다.** 서로
모순이라 믿을 수 없다. 상태 코드로 성공을 판정하지 않는 것과 같은 이유로,
소스가 주는 메타데이터도 내용과 대조되지 않으면 근거가 못 된다.

**② 애플의 페이지 상한은 10이다. 11페이지부터 HTTP 400.**

러너의 `MAX_PAGES_PER_TARGET` 은 20이다. **어댑터가 먼저 멈추지 않으면
11페이지에서 400 을 받고 러너가 그 타깃을 `failed` 로 찍는다** — 정상적인
경계가 고장으로 기록되는 형태다(§7.2). `nextRequest` 가 10페이지를 넘으면
null 을 돌려준다.

**③ `id.label` 이 안정 리뷰 ID 다.** 지문 1순위가 채워지므로 폴백 조합 키가
생기지 않는다(실측 35건 전부 `fallbackKeys 0`).

**④ 다나와에 없던 축이 온다 — `im:version`(앱 버전).**
"어느 버전부터 불만이 늘었나"를 볼 수 있다. 제목과 함께 본문 앞에 붙여
분석 단계가 읽게 했다.

**⑤ 판매처(`seller`)는 없다.** 다나와가 몰별 비교 축을 주는 것과 다르다.
카테고리마다 얻는 축이 다르다는 뜻이고, 분석 단계가 이걸 전제하면 안 된다.

### `product_ref` 형식 — 국가를 담는다

`kr:1459969523` 또는 `1459969523`(국가 생략 시 `kr`).

**같은 앱이라도 스토어 국가마다 리뷰가 다르다.** 한국 사용자 목소리를 보려면
`kr`, 글로벌 반응을 보려면 `us` 다. 앱 ID 가 숫자가 아니거나 국가 코드가
2자가 아니면 요청 자체를 만들지 않는다(URL 을 잘못 붙여넣는 경우를 막는다).

### 다나와에서 잡힌 두 버그를 같은 형태로 막았다

**커서 대칭성을 값으로 검사한다.** `nextCursor !== null` 만 보면 §13 의
버그 1(페이지 전진이 `1,2,2,2`)을 못 잡는다. 커서를 물려 4번 돌려
`1,2,3,4` 로 전진하는지 확인한다.

**본문·리뷰ID 가 없으면 파싱 실패로 센다.** 제목만 읽고 "정상 수집"으로
보고하던 §7.1 사례 1번을 만들지 않는다. "0건 파싱"과 "항목은 보이는데
0건"을 가른다.

### 검증

- `review-appstore-selftest` **42건** — 실제 응답 픽스처 2종
  (`fixtures/appstore/page1.json` 35건 / `page-empty.json`)
- `review-runner-selftest` 82 → **97건** — **어댑터 × 러너 경계면**을 따로
  테스트했다. 다나와 때와 같은 이유다: 부품이 각각 통과해도 붙이면 안 될 수
  있고, 그걸 실행에서 처음 알면 늦는다
  - 특히 "애플 페이지 상한을 넘지 않는다(400 요청 0건)" 를 검사한다
- 전체 셀프테스트 **561건** 통과, tsc 통과

### 아직 안 된 것 — 소스 등록은 사람이 한다

`review_sources` 에 `appstore` 행이 없으면 타깃을 한 건도 못 넣는다
(`review_targets.source_key` / `analysis_inputs.source_key` 가 둘 다 FK 로
`review_sources(key)` 를 본다).

```
supabase/migrations/20260902000001_appstore_source.sql        (+rollback)
```

🟢 비파괴(새 행 1개). ⛔ Claude 가 실행하지 않는다.

`min_interval_ms=2000` — 애플이 공개한 요청 제한이 없다. 다나와(4000)보다는
짧게 두되 1초 미만으로 내리지 않는다. 남의 서버다.

**실제 네트워크 통합은 미검증이다.** 픽스처로 어댑터×러너를 붙여 봤지만,
러너가 실제 애플 서버에 대고 도는 것은 소스 등록 후 첫 dry-run 에서 확인된다.

### 다음 — 타깃 매핑

소스가 등록되면 12개 프로젝트 중 **앱이 있는 것**에 App Store 타깃을 추가한다.
다나와 타깃과 공존한다(`review_targets` 는 `(project_id, source_key,
product_ref)` UNIQUE 라 같은 프로젝트에 소스별로 하나씩 붙는다).

현재 12개는 전부 물리 제품이라 앱이 없다. **SaaS 프로젝트를 새로 만드는
것이 자연스러운 다음 단계**이고, 그건 승인 대기 중인 카테고리 확장 트랙이다.

---

## 커뮤니티 소스 2종 — damoang · 82cook (2026-09-16)

실측 근거: `docs/review-source-findings.md`
"커뮤니티 소스 실측 — damoang · 82cook (2026-09-16)"
사람 결정: `docs/strategy-principles.md` SP-025(리스크 수용) · SP-026(러너 결함)

### 무엇이 다른가 — 수집 단위가 상품이 아니다

다나와는 상품 1개 = 타깃 1개, App Store 는 앱 1개 = 타깃 1개다. 커뮤니티는
**글 1건 = 타깃 1개**다. 그래서 `product_ref` 형식이 새로 하나 생겼다.

| 소스 | product_ref | 예 |
|---|---|---|
| danawa | `<pcode>` | `12345678` |
| appstore | `<국가>:<앱ID>` | `kr:1459969523` |
| hackernews | `q:<키워드>` | `q:Notion` |
| **damoang** | **`url:<글 경로>`** | `url:/free/7341567` |
| **82cook** | **`url:<글 경로>`** | `url:/entiz/read.php?num=4239440` |

파싱 결과는 **글 본문 1건 + 댓글 N건 = 리뷰 N+1건**이다. 댓글은 `storyId` 에
글 경로가 들어가고 본문 항목은 `storyId=null` 이다(hackernews 의 스레드 개념과 같다).

### ⚠️ `url:` 은 경로만 받는다 — SSRF 경계

`product_ref` 는 DB 행이고 사람이 손으로 넣는다. 호스트를 거기 담게 하면
**우리 수집기가 임의의 주소를 대신 때려 주는 장치**가 된다. 그래서 호스트는
어댑터가 상수로 들고(`HOST`), ref 에서는 경로만 받는다.

검증은 `lib/review/adapters/url-ref.ts` 한 곳에 있다. DRY 때문이 아니라
**우회 구멍이 나왔을 때 고칠 자리가 하나여야** 해서다. 복사해 두면 한쪽만
고치고 다른 쪽은 뚫린 채로 남는다.

거부 대상: `url:` 접두 없음 / `/` 로 시작하지 않음 / `..` · `//` · `@` · `\` 포함 /
공백·제어문자 포함. 쿼리스트링은 허용한다(82cook 글 주소가 쿼리형이다).

### 1글=1요청 — 댓글 페이지네이션을 붙이지 않았다

`parse()` 가 항상 `nextCursor: null` 을 낸다. 러너가 그 타깃을 `exhausted` 로
닫고 다시 방문하지 않는다(전 소스 공유 semantics, 새 결함이 아니다).
**나중에 달린 댓글은 자동으로 다시 받지 않는다** — 사람이 DB 에서
`status='active'` 로 되돌려야 한다. 자동 재활성화는 만들지 않았다(YAGNI).

붙이고 싶어지면 **SP-026 을 먼저 고쳐라.** 다모앙 robots 가
`Disallow: /*?page=` 를 걸어 뒀는데 러너가 robots 판정에 쿼리를 안 넘겨서
(`runner.ts:153` — `u.pathname` 만), 우리가 `?page=N` 을 만들면 안전장치가
그 위반을 **못 막는다**(§7.2 그대로의 상황이다).

그때까지의 방어는 어댑터 쪽에 있다:
- damoang: `?page=` 를 아예 만들지 않는다(셀프테스트가 고정한다).
- 82cook: robots 가 콕 집어 금지한 URL 1건(`?bn=15&num=1166440&page=6`)을
  `parseProductRef` 가 직접 거부한다. 파라미터 순서를 바꿔도 잡는다.

### "댓글 0건"과 "댓글 영역 소실"을 가르는 법

두 사이트 다 **댓글 수를 화면에 찍어 준다.** 이게 §7.1 을 지킬 수 있게 해 준 열쇠다.

| 소스 | 개수 마커 | 항목 앵커 | 본문 |
|---|---|---|---|
| damoang | `댓글 <span>(N)</span>` | `<li id="c_123" class="comment-item">` | `<div class="comment-body">` |
| 82cook | `<strong class="total_reple">N</strong>` | `<li data-rn="123" class="rp">` | `repleFunc` div 가 닫힌 뒤 |

판정 규칙:
- 마커도 없고 항목도 없다 → **댓글 영역 소실**(`parseFailures++`)
- 마커가 0 이고 항목도 0 → **정상**(실패 아님)
- 마커가 N 인데 항목이 M(<N) → 못 읽은 `N-M` 건을 실패로 센다
- 항목은 있는데 본문 컨테이너가 없다 → 그 건을 실패로 센다
- 컨테이너는 있는데 알맹이가 없다(이모티콘만 단 댓글) → **실패 아님**, 저장만 건너뛴다

글 본문도 같다: damoang 은 JSON-LD `DiscussionForumPosting` 이, 82cook 은
`#articleBody` 가 없으면 실패다.

### externalId — 폴백으로 후퇴하지 않는다

- damoang 댓글: `<글경로>#c_7341623` (앵커 id)
- 82cook 댓글: `<글경로>#41169700` (`data-rn`)
- 본문: `<글경로>`

글 경로를 접두로 붙이는 이유는 앵커 번호가 **글 안에서만** 유일할 수 있어서다.

composite 지문으로 후퇴하지 않는다. 후퇴하면 같은 사람이 같은 날 같은 글에 두 번
쓴 댓글의 두 번째가 조용히 사라진다. id 를 못 구하면 그 건을 실패로 센다.
라이브 프로브에서 `fallbackKeys=0` 을 확인했다 — 전건 id 를 읽었다.

### 셀렉터 — 문서가 아니라 실측이 근거다

설계 단계에서 짐작했던 것과 실물이 달랐다. **`articleBody` 는 82cook 셀렉터이고
damoang 셀렉터가 아니다**(damoang 문서에서 0건). damoang 은 대신 schema.org
JSON-LD 를 실어 준다. 픽스처는 전부 2026-09-16 실측 응답에서 깎았다.
...
맞췄다** — 픽스처가 스스로 모순되면 구조검사 테스트가 거짓 실패한다.

파서가 실제로 검사를 하고 있는지는 변이 8종(컨테이너 검사 제거, 마커 대조 제거,
연도 피벗 제거, robots 금지목록 제거, IP 경계 제거 등)을 넣어 전부 실패하는 것으로
확인했다. 통과하는 테스트보다 **틀렸을 때 실패하는 테스트**가 필요하다.

### 소스 등록

```
supabase/migrations/20260917000001_review_sources_community.sql  (+rollback)
```

🟢 비파괴(새 행 2개 + 컬럼 주석). DDL·백필 없음. ⛔ Claude 가 실행하지 않는다.

`min_interval_ms=3000` · `daily_request_cap=100` — 공식 API 가 아니라 개인이
운영하는 커뮤니티라 기존 소스(1000~2000ms / 200)보다 보수적으로 잡았다.
1글=1요청이므로 상한 100 은 하루 100글이다.

**둘 다 `enabled=false` 로 들어간다.** 이용약관 원문 확인이 끝난 뒤 사람이 켠다.
