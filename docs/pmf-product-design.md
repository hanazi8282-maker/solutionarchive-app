...
## 8. 가격 · WTP · 비용 (2026-09-21)

**이번 회차에 한 것은 신호 수집 하나뿐이다.** 진단 결과 화면(`/analyze/[id]/result`) 맨 아래
"이 진단, 얼마짜리였나" 카드가 답을 받아 `public.wtp_signals` 에 이력으로 쌓는다
(`lib/analysis/wtp.ts` · `app/api/analyze/wtp/route.ts`). 덮어쓰지 않는다 — 생각이 바뀌면 행이 하나 더 쌓인다.
답하지 않은 것(행 없음)과 "안 내겠다"(`would_pay=false`)는 다른 사건이다.

**안 한 것 — 가격 모델과 소스 비용 벤치마크.**

- **가격 모델(일회성 저가 + 선택적 구독, 재사용률 40% 기준 전환)** — 구현하지 않는다.
  화면 어디에도 가격이 없고, 위 카드의 답으로 가격이 정해지지도 않는다. 가격 정책은
  **사업 방향 결정**이라 사람이 정한다(CLAUDE.md §10.2 사람 판단 예외). 세션이 숫자를 고르지 않는다.
- **소스 비용 벤치마크(소스 비용 > ARPU 의 15% / 30% → 아키텍처 재검토)** — 구현하지 않는다.
  ARPU 도 매출도 0이고 분모가 없다. 소스별 LLM·인프라 비용을 나눠 기록하는 원장도 없어
  분자도 없다. 지금 이 규칙을 코드에 넣으면 상수 두 개가 아무것도 재지 않은 채 "지켜지고 있는"
  것처럼 보인다(§7.1 — 확인 불가를 양성으로 접지 않는다).

**선행 조건 — 이게 생기면 그때 다시 본다.**

- 재사용률을 재려면: 프로젝트를 `seller_profile` 단위로 소유시켜, 한 사람이 진단을 몇 번
  다시 돌리는지 세는 자리(지금은 프로젝트가 사람에 묶여 있지 않아 "재사용"을 셀 수 없다).
- 소스 비용을 재려면: 소스별 LLM·인프라 비용 원장(호출 1건 = 어느 소스 · 토큰 · 금액).
  `analyze-llm-budget` 의 상한은 요청·하루 단위라 소스별로 쪼개지지 않는다.
- 둘 다 생긴 뒤에도 임계값(40% · 15% · 30%) 자체는 사람이 정한다. 실측이 재료일 뿐이다.
