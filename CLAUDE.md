...
1회성 예외(남헌 승인, 클로드코드가 직접 적용)는 `docs/migration-exceptions.md` 에 쌓는다.
2026-09-15 기준 6회(09-10, 09-11, 09-12, 09-14 ×2, 09-15). **원칙 문구(§10.1) 자체는 변경하지 않는다.
다음 마이그레이션부터는 별도 승인이 없는 한 다시 원칙대로 사람이 적용한다.**
