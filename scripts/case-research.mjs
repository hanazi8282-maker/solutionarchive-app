...
■ 반드시 답할 것 — **이 순서대로 답해라**
  ...
  3.5. **사고의 흐름 1건 이상** — 어떤 관찰에서 어떤 추론으로 그 결정에 갔나.
     ...
     ★ 1인칭·정황 근거를 찾는 곳 하나 더 — **Hacker News 댓글 검색**:
         hn.algolia.com/?query=<회사명>&type=comment&sort=byPopularity
       창업자·직원·초기 사용자가 "왜 그렇게 했는지"를 언론 인터뷰보다 먼저,
       더 날것으로 적어 두는 자리다. 특히 Show HN 스레드를 보라.
       ⚠️ 등급 규칙(두 축은 **서로 독립**이다. 같이 움직이지 않는다):
         · \`source_tier='secondary'\` — HN 댓글은 언제나 2차다. 댓글 자체가
           1차 문서는 아니고, 작성자가 당사자여도 그건 아래 축에서 표시한다.
         · \`is_self_reported=true\` 는 **작성자가 당사자일 때만**. 예: Show HN
           스레드에서 원글쓴이(제품 만든 사람)가 단 댓글, 회사 계정으로 밝힌 댓글.
           제3자가 "내가 듣기로는" 하는 댓글은 \`false\` 다.
         · 당사자인지 확인이 안 되면 \`false\` 로 둔다. 확인 불가를 양성으로
           접지 않는다(§7.1).
