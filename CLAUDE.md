...
- **마이그레이션 적용** — 무인 루프에는 그 권한이 없고 도구도 부여하지 않는다
  ...
  보고에 명시한다. **누가 적용할 수 있는지는 §10.2 가 정한다** — 무인 루프는
  어떤 경우에도 적용하지 않는다.
...
### 10.2 마이그레이션을 누가 적용하나 (2026-09-17 개정)

**가르는 기준은 "적용 전에 남헌에게 물을 수 있는가"다.** 물을 수 있는 자리는
승인을 받고 적용한다. 물을 수 없는 자리는 어떤 경우에도 적용하지 않는다.

| 주체 | 적용 | 조건 |
|---|---|---|
| 남헌 | ✅ | Dashboard SQL Editor 또는 `supabase db query --linked -f` |
| 남헌이 직접 모는 대화형 세션 | ✅ | **적용 전 명시적 승인 1회.** 아래 절차를 지킨다 |
| CMO · CTO 역할 세션 | ✅ | 남헌에게 직접 묻지 않는다(`_principles.md §8`). CEO-STAFF 에 올려 승인이 내려온 뒤에만 |
| 서브에이전트(Task) | ⛔ | 사람에게 물을 수 없다. 파일만 만들고 "미적용" 보고 |
| 무인 루프(GitHub Actions) | ⛔ | 물어볼 사람이 없다. 도구·권한이 설정 레벨에서 없다 |

**승인을 받은 세션이 지킬 절차**

1. **무엇을 적용하는지 먼저 말한다** — 파일 경로, 대상 프로젝트, 비파괴 여부,
   롤백 파일 유무. 그 위에서 승인을 받는다. 침묵·추정 승인은 승인이 아니다.
...
적용 이력은 `docs/migration-exceptions.md` 에 쌓는다. 2026-09-17 개정 전까지는
"사람만 적용"이 원칙이었고 1회성 예외 6회(09-10, 09-11, 09-12, 09-14 ×2, 09-15)가
그 문서에 기록돼 있다. 개정 이후의 적용은 예외가 아니라 이 절차를 따른 정상 경로다.
