...
### 11.6 폴백 블록이 풀렸다 — 원인은 모른다 (2026-09-02)

§11.5 이후 사용자가 다시 시도하니 **저장이 됐다.** 브라우저에서 직접
새로고침해 확인한 상태:

- 파라미터 설정에 스킬(솔루션아카이브 인사이트 저장)이 선택된 채 유지
- 봇 응답 `스킬데이터 사용` 유지
- 이전에는 새로고침하면 즉시 원상복귀됐는데 이번에는 유지됐다

**🔴 왜 풀렸는지 모른다. 이걸 "해결됨"으로만 적으면 안 된다.**

우리가 한 일 중 무엇이 상태를 바꿨는지 특정하지 못했다. 사용자가 관찰한
것은 **봇 상태가 `준비` → `실행` 으로 바뀌어 있었다**는 것이고, 시점상
일반 블록(`카카오웹훅테스트`)을 만들고 전체 배포한 것과 겹친다.
**이는 사용자의 추측이고 확정이 아니다** — 배포가 어떤 검증 상태를
정리했을 가능성이 있다는 정도다. 인과를 증명하지 않았다.

따라서 **재발 가능성을 열어 둔다.** 같은 증상이 다시 나오면 §11.5 의
증상 기술과 아래 문의 초안을 그대로 쓸 수 있다.

#### 노출 면이 바뀌었다 (기록)

폴백에 스킬이 붙었으므로 **이제 채널로 오는 모든 메시지가 우리 웹훅에
도달한다.** 전에는 정확매칭 블록에 걸린 발화만 왔다. 저장 게이트는
`KAKAO_ALLOWED_USER_IDS` fail-closed 그대로라 권한은 안 늘었지만,
**채널을 추가한 다른 사람의 발화도 거부 로그에 그 사람의 botUserKey 와
함께 남는다.** 진단에 필요해서 그렇게 두지만, 알고 있어야 한다.

#### 보관 — 데브톡 문의 초안 (지금은 불필요, 재발 시 사용)

```
제목: [오픈빌더] 폴백 블록에 스킬 연결 시 "기본 블록(웰컴, 폴백, 탈출)은
      유일해야 합니다" 로 저장 실패

■ 대상
- 봇 이름: 솔루션아카이브
- 봇 ID: 6a970c6d9af9adecde692a6c
- 같은 계정에 다른 봇 1개가 더 있습니다("sip 테스트용 봇").

■ 증상
[시나리오] → [폴백 블록] → [파라미터 설정]에서 스킬을 선택하고
[봇 응답]을 "스킬데이터 사용"으로 지정한 뒤 저장하면 아래 경고가 뜨고
저장되지 않습니다.

  "기본 블록(웰컴, 폴백, 탈출)은 유일해야 합니다."

새로고침하면 변경 전 상태로 돌아갑니다(서버에 반영되지 않음).
스킬의 "적용 블록수"는 계속 0으로 남습니다.

■ 재현 절차
1. [시나리오] → [폴백 블록] 진입
2. [파라미터 설정]에서 등록된 스킬 선택
3. [봇 응답]을 "스킬데이터 사용"으로 지정
4. 저장 → 위 경고 발생, 저장 실패

■ 재현 조건 (여러 번 확인)
- 브라우저를 완전히 종료하고 새 세션으로 다시 로그인한 뒤, 해당 봇만
  단독 탭에서 열고 다른 봇으로 전환하지 않은 상태에서도 재현됩니다.
- 새 블록을 만들지 않았고, 기존 폴백 블록을 그 자리에서 수정만 했습니다.
- 시나리오 좌측 목록에는 웰컴/폴백/탈출 블록이 각각 1개씩만 보입니다.
  (다만 이는 화면상 확인이며, 실제 중복 여부는 저희가 알 수 없습니다.)

■ 참고 — 스킬 서버는 정상 동작합니다
같은 스킬에 대해 [스킬 테스트]는 "올바른 스킬 서버 응답입니다" 로
통과합니다. 서버 로그에도 같은 시각 요청이 정상 수신된 것을 확인했습니다.

■ 여쭙고 싶은 것
1. 이 경고 메시지가 발생하는 정확한 조건은 무엇인가요?
2. 저희 봇의 기본 블록이 실제로 중복 상태인지 확인해 주실 수 있나요?
3. 중복이라면 어떻게 정리할 수 있나요? 기본 블록은 삭제가 불가능한
   것으로 안내되어 있어 저희 쪽에서 정리할 방법을 찾지 못했습니다.
```

#### 정리 대기 목록 — 지금 건드리지 않는다

폴백이 **며칠 안정적으로 도는 것을 확인한 뒤** 정리한다. 원인을 모르는
채로 풀렸으므로, 지금 주변을 건드리면 무엇이 되돌렸는지도 모르게 된다.

1. **임시 우회 블록** `웹훅테스트` + 발화 `카카오웹훅테스트` — `user.id`
   확보라는 목적을 다했다. 폴백이 모든 발화를 받으므로 더는 필요 없다
2. **빈 시나리오 2개** (`시나리오 01`, `시나리오 02`) — 브라우저 조작 중
   `+시나리오` 오클릭으로 생긴 찌꺼기. 블록이 없어 무해하다
3. `.vercel/` (CLI 링크 산출물) — `.gitignore` 33행에 이미 있어 커밋
   위험 없음. 로컬 전용

**2번을 지금 지우지 않는 이유**를 특히 적어 둔다. 풀리지 않았던 오류가
하필 **시나리오·블록의 유일성 검증**이었고, 빈 시나리오가 생긴 시점이
그 부근이다. 관련이 있다는 증거는 없지만 **없다는 증거도 없다.**
지금 지웠다가 증상이 재발하면 원인 후보가 하나 더 늘어난다.

#### 아직 검증 안 된 것 — 다음 단계

1. **폴백 경로로 게이트를 실제로 타는가** — 정확매칭 블록과 겹치지 않는
   평범한 메시지를 보내 확인한다. `[kakao-webhook] utterance(...)` 가
   로그에 찍히면 통과다
2. **첫 end-to-end 저장** — `<Threads URL> <본문 40자 이상>`. 여기서
   처음으로 `saved_examples` 에 행이 생긴다
3. ⚠️ **첫 저장은 실제로 남기고 싶은 글로 해야 한다.** 그 행은 오늘 밤
   인사이트 루프가 LLM 으로 분석해 `insight_patterns` 에 넣는다. 아무 말이나
   넣으면 근거가 오염되고, 지우려면 밤 전에 손으로 지워야 한다

### 11.7 텍스트 전용이다 — 이미지·사진은 파이프라인에 들어오지 않는다

**현재 한계를 명시한다. 계획이 아니라 사실이다.**

`app/api/insight/kakao-webhook/route.ts` 는 `userRequest.utterance`
**문자열 하나만** 읽는다. 첨부·미디어 필드를 보지 않는다. 그 뒤 단계도
전부 텍스트다 — `parseShare()` 가 URL 과 본문을 가르고, `raw_text` 에
문자열이 들어가고, `lib/insight/llm.ts` 가 그 문자열을 분석한다.

따라서:

- **사진만 보내면 아무 일도 일어나지 않는다.** 저장도 분석도 안 된다
- **글에 딸린 이미지의 내용은 분석에 반영되지 않는다.** 텍스트만 본다
- 이미지 안에 글이 있어도 읽지 않는다 — **OCR 은 없다**

**이미지 설명·OCR 도입 계획은 현재 없다.** 넣는다면 새 외부 의존(비전
모델)과 새 과금이 붙고, 원문 30일 폐기 정책(§9)이 이미지 파생물에도
적용되는지부터 정해야 한다. 지금은 그 판단을 하지 않았다는 뜻이지
"검토했고 기각했다"는 뜻이 아니다.

사용자에게는 "사진은 분석되지 않는다"로 안내한다.

### 11.8 ⚠️ `vercel logs` 는 현재 git 브랜치로 조용히 필터링한다

실제로 당했다. feat 브랜치에 체크아웃한 상태로

```
vercel logs --environment production --since 5h
```

를 돌렸더니 `No logs found for hanazi-s-projects/solutionarch on branch
docs/kakao-fallback-resolved` 가 나왔다. **프로덕션 요청이 실제로는 여러
건 있었는데도 0건으로 보였다.**

`--branch` 는 *"defaults to current branch for a linked project"* 다.
프로덕션 로그를 보는데 로컬 브랜치가 필터로 들어온다. 그리고 출력이
"에러"가 아니라 **"No logs found"** 라서, 그대로 읽으면 "요청이 안 왔다"는
**정반대 결론**이 나온다 — §7.1 이 말하는 형태다(음성과 확인 불가를
구분하지 못하는 검사).

**규칙: 프로덕션 로그를 볼 때는 `--no-branch` 를 반드시 붙인다.**

```
vercel logs --environment production --no-branch --since 5h -x -n 300
```

두 가지 더:

- **CLI 는 로그를 stdout 이 아니라 stderr 로 낸다.** `> file` 로만 받으면
  빈 파일이 된다. `2> file` 로 받아야 한다
- 시각은 **KST 로 표시**된다. Vercel MCP 는 UTC 다. 둘을 섞어 보면
  9시간이 어긋난다

### 11.9 🟢 첫 end-to-end 성공 — 그리고 첫 실사용이 설계 전제를 깼다 (2026-09-02)

**트랙 1 의 입구가 처음으로 열렸다.** 몇 주째 0건이던 `saved_examples` 에
실제 행이 들어왔다.

#### 성공 경로 (실측)

봇 재배포(v1.0 → v1.1)가 마지막 조각이었다. **폴백에 스킬을 연결한 저장이
v1.0 배포 *이후* 였기 때문에, 살아 있는 봇은 계속 "폴백에 스킬 없음"
상태였다.** 오픈빌더는 저장과 배포가 분리돼 있고, 실제 채팅에 도는 것은
배포된 버전이다. 그래서 메시지가 폴백으로 가고도 우리 웹훅을 부르지 않았고,
로그에는 아무것도 안 남았다 — **"서버가 멀쩡한데 아무 일도 안 일어나는"
형태**라 원인을 로그에서 찾을 수 없었다.

세 건 모두 게이트를 통과했다(KST):

```
22:38:47  utterance(233자)  본문만 (URL 없음)
22:38:57  utterance(139자)  URL만
22:39:10  utterance(374자)  URL + 본문
```

저장된 행:

- 멱등키 `url:https://www.threads.com/@choi.openai/post/DcxAc6Vj0Jx`
  — **추적 파라미터(`?xmt=…&slof=1`) 정상 제거**
- `source_url` 채워짐(원본 URL 그대로), `raw_text` 230자, `pending`
- **§11.3 설계가 실제로 동작했다**: URL 만 온 22:38:57 에 행이 생기고,
  22:39:10 에 같은 링크가 다시 오자 **새 행이 아니라 원문만 채워졌다**

#### 🔴 그런데 행이 2개가 됐다 — 멱등키 네임스페이스가 갈린다

`idempotencyKey()` 는 URL 이 있으면 `url:…`, 없으면 `text:…` 를 쓴다.
**첫 전송에 URL 이 없어서 본문으로 키가 잡혔고, 뒤이어 온 URL 은 다른
키가 됐다.** 같은 글이 두 행이 됐다.

```
행1  text:큰 거 온다.. 오픈AI가…   source_url=null   raw_text 230자
행2  url:…/post/DcxAc6Vj0Jx        source_url=있음   raw_text 230자
```

**왜 위험한가.** 오늘 밤 루프가 두 행을 각각 분석하면 같은 인사이트가 두 번
나오고, `patternize` 가 그것을 **서로 다른 두 근거로 세어
`evidence_count` 가 2** 가 된다. 이 프로젝트의 승격 기준이
"2건 이상 반복 관찰"이므로 **글 하나가 그 기준을 거짓으로 충족한다.**
`lib/insight/capture.ts` 주석이 반복해서 경계하는 바로 그 형태가,
캡처 경로가 아니라 **발화 순서**에서 났다.

**설계 전제가 틀렸다.** "한 메시지에 URL 과 본문이 같이 온다"를 전제했는데,
첫 실사용자는 **본문 따로, URL 따로** 보냈다. 사람이 그렇게 보내는 게
자연스럽다.

#### 이번 처리 — 손으로 지웠다

사용자 승인 후 행1(`6102c7bd-bf39-4ab7-83d6-6a7e49f06c88`) 삭제.
`source_url` 이 없는 쪽을 버리고 URL·본문이 다 있는 행2 를 남겼다.
삭제 전 대상 id 를 다시 조회해 확인했고, 삭제 후 전량이 1행인 것도 확인했다.

#### 고칠 방향 (결정하지 않았다)

- `url:` 행을 만들 때 **같은 `raw_text` 를 가진 `text:` 행이 있으면 병합**
  — 다만 "같다"의 판정(정규화·부분일치)을 정해야 하고, 매 저장마다 본문
  조회가 붙는다
- 또는 **웹훅 응답에서 안내를 바꾼다** — 본문만 온 경우 "링크도 함께 보내면
  같은 글로 묶입니다"를 그 자리에서 말한다. 코드 변경이 작고 DB 조회가 없다
- 지금은 **운영으로 막는다**: 한 메시지에 URL 과 본문을 **같이** 보낸다

#### 남은 것

`saved_examples` 1행 `pending`. **오늘 밤 KST 03:41 인사이트 루프가 이
행을 처음으로 분석한다** — 크론 시각 이동(§13.21)의 첫 발화이자, DB 쓰기
경로의 첫 실전 검증이기도 하다. 두 가지가 같은 밤에 걸려 있다.

### 11.10 내일 아침(2026-09-03) 확인 목록 — 두 가지가 같은 밤에 걸린다

이 절은 **세션이 끊겨도 그대로 실행할 수 있게** 적는다. 오늘 밤 하나의
실행에 서로 다른 두 검증이 겹쳐 있다.

- **크론 시각 이동(§13.21)의 첫 발화** — `0 18`→`37 17`, `0 19`→`41 18` 로
  옮긴 뒤 아직 한 번도 안 떴다. 머지가 09-01 19:58 UTC 였고 새 시각은 그날
  이미 지난 뒤였다. **첫 발화는 09-02 17:37 / 18:41 UTC (KST 09-03 02:37 /
  03:41).**
- **DB 쓰기 경로의 첫 실전** — `saved_examples` 에 처음으로 진짜 행이 있다
  (§11.9). 지금까지 모든 루프 실행은 dry-run 이거나 0건이었다.

#### 확인 순서

**1) 스케줄로 실제 발화했는가** (수동 실행과 반드시 구분)

```
gh run list --event schedule --limit 10
```

09-02~03 날짜 행이 **2건**(Review Collect / Insight Loop) 새로 있어야 한다.
없으면 정각 혼잡 가설이 틀린 것이고 다른 데를 봐야 한다.

**2) 인사이트 루프 — 첫 실제 분석**

- `saved_examples` 그 행의 `analysis_status` 가 `pending` → **`analyzed`**
- `insight_patterns` **1건**. ⚠️ **2건이면 안 된다** — 2건이면 §11.9 의
  중복이 어딘가에서 되살아난 것이다
- `insight_loop_runs` 에 행이 1건 쌓였는가. 0행이면 DB 쓰기 경로가 여전히
  미검증이다(스케줄 실행은 dry-run 이 아니다)

**3) 리뷰 수집 — 첫 실적재**

- `review_collection_runs` / `review_fingerprints` 증가분
- ⚠️ **타깃은 12개가 아니라 10개만 돈다.** 스케줄 실행은 `inputs.targets`
  가 비어 `--targets=10` 이 먹는다(`nightly-review-collect.yml:78`).
  `review_collection_runs` 의 타깃 수가 **10이면 상한**이고, 10 미만이면
  그때가 조사 대상이다. "12개 안 돌았다"를 고장으로 읽지 마라(§7.2)
- 폐기 배치는 **dry-run 이 정상**이다. 리포 변수 `REVIEW_PURGE_APPLY` 가
  없으므로(`gh variable list` = 0개) "후보 N건 / 폐기 0건"이 맞다.
  지웠다고 나오면 그게 이상한 것이다

**4) 실행 시간을 봐라.** 25~40초로 끝난 야간 루프는 도는 게 아니다(§7.2).
직전 수동 수집은 10분 27초였다.

#### 로그 조회 시 주의

`vercel logs` 는 **현재 git 브랜치로 조용히 필터링**한다(§11.8).
프로덕션을 볼 때는 반드시:

```
vercel logs --environment production --no-branch --since 12h -x -n 300 2> out.txt
```

`2>` 다. CLI 는 stdout 이 아니라 **stderr** 로 낸다. 시각은 **KST** 표시고
Vercel MCP 는 UTC 다 — 섞어 보면 9시간 어긋난다.
