...
import { DEPARTMENTS, PENDING_INTEGRATIONS, SCENARIO } from "../../company.config";
import {
  CEO_REPORT_SPOT,
  CEO_SEAT,
  COLS,
...
/** 하루 단계 이름 — company.config.ts 의 SCENARIO 에서 온다 */
const PHASES: readonly string[] = SCENARIO.phases;

/**
 * 연동 대기 부서와 그 사유는 company.config.ts 의 PENDING_INTEGRATIONS 하나에서 유도한다.
 * 그 객체가 비어 있으면 차단 부서도 없고, 관련 장면·대사도 전부 건너뛴다.
 */
const BLOCK_REASON: Record<string, string> = PENDING_INTEGRATIONS;
const BLOCKED_DEPTS = new Set(Object.keys(BLOCK_REASON));

/** 대표 지시창에서 대답하는 사람 = 비서실 리드 */
const SECRETARY = DEPT_LEAD.secretary;

/**
 * 지시창에서 부서를 찾을 때 쓰는 키워드 — 부서 이름·약칭과 그 팀 직원 이름·콜사인에서
 * 런타임에 만든다. 긴 키워드부터 검사해 짧은 단어가 먼저 걸리는 오판을 줄인다.
 */
const DEPT_KEYWORDS: [string, string][] = DEPARTMENTS.flatMap((dept) => {
  const crew = STAFF.filter((s) => s.deptId === dept.id);
  const words = [
    dept.name,
    dept.name.replace(/팀$/, ""),
    dept.short,
    dept.short.split(".")[0],
    ...crew.map((s) => s.name),
    ...crew.map((s) => s.callsign ?? ""),
  ];
  return words
    .filter((word) => word.length >= 2)
    .map((word) => [word.toLowerCase(), dept.id] as [string, string]);
}).sort((a, b) => b[0].length - a[0].length);

/** 부서가 오늘 하는 작업 라벨·완료 대사 (config 에 없으면 부서 소개로 대체) */
function stepOf(deptId: string) {
  return SCENARIO.steps[deptId] ?? { label: DEPT_BRIEF[deptId]?.task ?? "업무", done: "" };
}
...
export class Company {
  ...
  /**
   * 화면에 실제로 보이는 값이 바뀔 때만 올라간다.
   * page.tsx 는 이 값이 바뀐 틱에만 snapshot() 을 새로 만든다 — 안 그러면
   * 초당 5~6번 같은 내용의 새 객체가 만들어져 아래 useMemo 가 전부 무효화된다.
   */
  version = 0;
  private stamp = "";
  ...
  reset() {
    ...
    this.pushChat("staff", SECRETARY.name, `대표님, ${SECRETARY.name}입니다. 궁금한 건 여기에 바로 물어보세요.`);
  }
  ...
  private *dayScript(): Generator<number | (() => boolean), void, void> {
    ...
    // ② 기획·설계
    this.phaseIndex = 2;
    yield* this.runDept("research", 6.5);

    // ③ 연동 대기 점검 — PENDING_INTEGRATIONS 가 비어 있으면 통째로 건너뛴다
    this.phaseIndex = 3;
    for (const deptId of BLOCKED_DEPTS) {
      const lead = DEPT_LEAD[deptId] ? this.agentById.get(DEPT_LEAD[deptId].id) : null;
      if (!lead) continue;
      const reason = BLOCK_REASON[deptId];
      this.stand(lead);
      this.say(lead, reason, 3);
      this.pushLog(roomOf(deptId).icon, `${roomOf(deptId).name}: ${reason}`, "lav");
      this.goto(lead, rand(LOUNGE_ROOM.loiter), "휴식");
      this.enqueue(lead, { k: "wait", dur: 4 });
      this.sitAtDesk(lead);
    }

    // ④ 아침 인수인계 회의
    yield* this.meeting(
      SCENARIO.handoff.title,
      SCENARIO.handoff.lines.map(([dept]) => DEPT_LEAD[dept].id),
      SCENARIO.handoff.lines.map(([dept, text]) => [DEPT_LEAD[dept].id, text] as [string, string]),
    );

    // ⑤ preview 실측 검증
    this.phaseIndex = 4;
    yield* this.runDept("strategy1", 7);

    // ⑥ 근본원인 진단
    this.phaseIndex = 5;
    yield* this.runDept("qa", 5.5);
    this.pushLog("🛡️", SCENARIO.logs.debugRound, "lav");

    // ⑦ 수용기준 확정 — 승인 안건 생성
    this.phaseIndex = 6;
    const summaryLead = this.agentById.get(DEPT_LEAD[SCENARIO.summary.dept].id)!;
    this.stand(summaryLead);
    this.say(summaryLead, SCENARIO.summary.line, 3);
    this.pushLog("💡", `${roomOf(SCENARIO.summary.dept).name}: ${SCENARIO.summary.log}`, "pink");
    yield 1.8;
    this.sitAtDesk(summaryLead);

    // ⑧ 대표 승인 회의
    this.phaseIndex = 7;
    this.deptStatus[SCENARIO.approval.pendingDept] = "승인 대기";
    this.approvalPending = true;
    this.turbo = false; // 대표 결정 지점에서는 즉시 정상 속도로 돌아온다
    this.meetingTitle = SCENARIO.approval.title;
    this.pushLog("📋", "대표 승인 대기: 오늘 결정할 안건 1개가 회의실에 올라왔어요.", "yellow");

    const approvers = SCENARIO.approval.crew.map((dept) => this.agentById.get(DEPT_LEAD[dept].id)!);
    const ceo = this.agentById.get("ceo")!;
    ...
    this.say(approvers[0], SCENARIO.approval.leadLine, 3.4);
    yield 2.4;
    this.say(approvers[approvers.length - 1], SCENARIO.approval.secretaryLine, 3.2);
    yield 2.2;
    this.say(ceo, SCENARIO.approval.ceoLine, 2.4);
    ...
    this.approvalPending = false;
    this.meetingTitle = null;
    this.say(ceo, SCENARIO.approval.approvedLine, 2.8);
    this.pushLog("✅", SCENARIO.approval.approvedLog, "mint");
    yield 1.6;
    ...
    // ⑨ CTO 관점 보고서 작성
    this.phaseIndex = 8;
    yield* this.runDept("strategy2", 7);

    // ⑩ 보고서 전달 → 무인 크론 2종 동시 가동
    this.phaseIndex = 9;
    for (const handover of SCENARIO.deliveries) {
      yield* this.deliver(DEPT_LEAD[handover.from].id, handover.to, handover.line, handover.reply);
    }

    this.startDept("reels", 8);
    this.startDept("carousel", 8);
    yield () => this.deptStatus.reels === "완료" && this.deptStatus.carousel === "완료";
    this.pushLog("🎬", SCENARIO.logs.cronDone, "mint");

    // ⑪ 초안 staging · 케이스 적립
    this.phaseIndex = 10;
    this.startDept("ops", 5);
    this.startDept("review", 5);
    yield () => this.deptStatus.ops === "완료" && this.deptStatus.review === "완료";
    this.pushLog("📦", SCENARIO.logs.archived, "mint");
    ...
    this.pushLog("📋", `${SECRETARY.name} 최종 브리핑 완료 — 완료 ${stats.done}팀 · 연동 대기 ${stats.blocked}팀`, "pink");
    yield 3;
    this.stand(seri);
    this.sitAtDesk(seri);
    yield this.allFree([seri]);
  ...
  /** 부서 업무 시작(비동기) — 작업 라벨은 config 에서 가져온다 */
  private startDept(deptId: string, dur: number) {
    const label = stepOf(deptId).label;
  ...
  private finishDept(deptId: string) {
    // 아직 큐에 할 일이 남은 팀원(자리로 걸어오는 중 포함)이 있으면 부서는 안 끝난 것이다.
    // 상태만 보면 리드가 먼저 도착해 일을 마쳤을 때 이동 중인 멤버를 놓친다.
    if (this.deptAgents(deptId).some((a) => a.status === "업무 중" || a.queue.length > 0)) return;
  ...
  private *runDept(deptId: string, dur: number) {
    const report = stepOf(deptId).done;
    this.startDept(deptId, dur);
    yield () => this.deptStatus[deptId] === "완료";
    const lead = this.agentById.get(DEPT_LEAD[deptId].id);
    if (lead && report) this.say(lead, report, 3.2);
    yield 1.2;
  }
  ...
  command(raw: string) {
    ...
    if (/승인|오케이|고고|진행해/.test(text) && this.approvalPending) {
      this.approve();
      this.pushChat("staff", SECRETARY.name, "승인 접수했습니다. 제작팀에 바로 넘길게요.");
      return;
    }
    ...
    this.pushChat(
      "staff",
      SECRETARY.name,
      `이렇게 물어보시면 제일 빨라요 — “현황 보고” / “왜 늦어져?” / “${DEPARTMENTS[0].name} 뭐해?” / “회의 소집” / “집중 모드” / “지금 브리핑”.`,
    );
  }
  ...
  private reportStatus() {
    if (!this.running && !this.dayComplete) {
      this.pushChat("staff", SECRETARY.name, "아직 출근 전이에요. ‘오늘 업무 시작하기’를 눌러주시면 전원 출근합니다.");
      return;
    }
    ...
    this.pushChat("staff", SECRETARY.name, lines.join("\n"));
  ...
  private reportDelay() {
    ...
    if (this.approvalPending) {
      ...
      const waiting = SCENARIO.approval.crew.map((dept) => DEPT_LEAD[dept].name).join("·");
      lines.push(
        `원인은 하나예요 — 대표님 결재 대기입니다. 회의실에서 ${waiting}가 ${waited}초째 기다리고 있어요.`,
      );
  ...
  private setFocusMode(on: boolean) {
    ...
    if (on) {
      this.recallAll(true);
      this.pushChat("staff", SECRETARY.name, "집중 모드 켰습니다. 커피·잡담 없이 전원 자리에서 업무만 봅니다.");
      this.pushLog("🎤", "대표 지시: 집중 모드 ON — 자율 휴식 중단", "yellow");
    } else {
      this.pushChat("staff", SECRETARY.name, "집중 모드 껐어요. 다들 숨 좀 돌리겠습니다 ☕");
  ...
  private recallAll(quiet = false) {
    ...
    if (!quiet) {
      this.pushChat("staff", SECRETARY.name, moved ? `${moved}명 자리로 복귀시켰습니다.` : "다들 이미 자리에 있어요.");
  ...
  private cheer() {
    ...
    this.pushChat("staff", SECRETARY.name, "대표님 한마디에 사무실 분위기가 확 살았어요 🩷");
    this.pushLog("🎀", "대표 격려 — 전 직원 사기 상승", "pink");
  }

  private convene() {
    if (this.side.gen) {
      this.pushChat("staff", SECRETARY.name, "앞선 지시를 아직 처리 중이에요. 끝나면 바로 잡겠습니다.");
      return;
    }
    const ids = Object.keys(this.deptStatus)
      ...
      .slice(0, 6);
    if (!ids.length) {
      this.pushChat("staff", SECRETARY.name, "지금은 다들 진행 중인 업무가 있어 소집이 어려워요. 잠시 뒤 다시 불러주세요.");
      return;
    }
    this.pushChat("staff", SECRETARY.name, `${ids.length}명 회의실로 소집했습니다. 각자 한 줄씩 보고할게요.`);
  ...
  private *conveneScene(ids: string[]): Generator<number | (() => boolean), void, void> {
    ...
    this.pushChat("staff", SECRETARY.name, "회의 마쳤습니다. 전원 자리로 복귀했어요.");
  }

  private briefNow() {
    if (this.side.gen) {
      this.pushChat("staff", SECRETARY.name, "앞선 지시를 처리 중이에요. 끝나고 바로 올라가겠습니다.");
      return;
    }
    const seri = this.agentById.get("secretary-lead")!;
    if (this.locked.has(seri.id)) {
      this.pushChat("staff", SECRETARY.name, "지금 다른 일정에 묶여 있어요. 마치는 대로 대표실로 가겠습니다.");
      return;
    }
  ...
  /** 질문에 등장한 부서·이름을 찾아낸다 (긴 키워드부터 검사) */
  private matchDept(text: string): string | null {
    const q = text.toLowerCase();
    return DEPT_KEYWORDS.find(([word]) => q.includes(word))?.[1] ?? null;
  }
  ...
  /** 스냅샷에 들어가는 값들의 지문을 떠서, 달라졌을 때만 version 을 올린다 */
  private syncVersion() {
    let statuses = "";
    for (const agent of this.agents) statuses += agent.status + ",";
    const next = [
      this.clockText(),
      this.phaseIndex,
      this.running,
      ...
      this.logSeq,
      this.chat.length,
      Object.values(this.deptStatus).join("·"),
      statuses,
    ].join("|");
  ...
  // ── 틱 ──────────────────────────────────────────────────
  tick(rawDt: number) {
    // 일시정지 중에도 지문은 갱신한다 — 멈췄다는 사실 자체가 화면에 보여야 한다
    if (this.paused) {
      this.syncVersion();
      return;
    }
