...
/**
 * 조사 계획이 "갭"으로 잡는 목표선. 매칭 성립 자체는 2곳으로도 된다(1곳은
 * 절반이 아니라 불가) — 이 상수를 올려도 `lib/cases/match.ts` 의 실제 매칭
 * 판정선은 안 바뀐다(그쪽은 이 상수를 참조하지 않는다, 확인함).
 *
 * 2 였을 때: 7개 병목 중 6개가 정확히 2곳(완충 0)이라 `coverageGaps()` 가
 * 항상 빈 배열을 냈다 — 라이브 3회(09-06/07/08) 전부 실패쿼터 슬롯만 채워져
 * 성공사례 조사가 0/day 로 굳었다(직접 `coverageGaps()` 를 돌려 확인, 추측
 * 아님). 3으로 올려 "매칭 성립"과 "완충 확보"를 분리한다 — 매칭은 여전히
 * 2곳부터 되지만, 계획은 3곳을 목표로 삼아 한 케이스가 반려/강등돼도 그
 * 병목이 바로 매칭 실패로 안 떨어지게 한다.
 *
 * ⚠️ 이건 "매일 성공사례 목표선을 3케이스로 올린다"는 콘텐츠 전략 판단을
 * 내포한다 — 남헌 승인, 2026-09-09.
 */
