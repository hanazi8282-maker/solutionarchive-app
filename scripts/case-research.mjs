...
■ 반드시 답할 것 — **이 순서대로 답해라**
  ...
  3. 옮기려면 뭐가 있어야 하나 (preconditions)
     ...
     ★ transfer_note·preconditions 는 **고등학생이 읽어도 막히지 않게** 쓴다
       (2026-09-17 남헌 — 인사이트는 채워지는데 전체적으로 읽기 어렵다는 판정).
       어려운 경영 용어는 그 자리에서 풀어 쓰고("유닛 이코노미가 깨졌다" 대신
       "팔수록 손해였다"), 한 문장에 조건을 두 개 이상 넣지 마라. 사실·수치·
       출처 귀속 문구는 그대로 둔다 — 쉽게 쓰는 것이지 근거를 흐리는 게
       아니다. 전문은 \`content/guides/인사이트-추출-기준.md\` §8.
       ⚠️ 이미 적립된 케이스는 다시 손대지 않는다 — 이 지시 이후 신규 조사부터다.
