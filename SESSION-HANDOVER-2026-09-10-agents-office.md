> **[정정 — 2026-09-12 추가]** 이 블록은 나중에 덧붙인 것이다. 아래 기존 내용은
> 그대로 두었다.
>
> **1. 루트 `CLAUDE.md` §2.1(ai-office 스택 예외 조항)을 담았던 커밋 `f6fee9f`
> 는 유실됐다.** 다른 세션이 그 28줄을 보존하려고 자기 브랜치
> `chore/kakao-connect-matchposts-ext-techdebt` 에 별도 커밋으로 담아 뒀는데,
> 그 브랜치가 두 차례 rebase 되면서 `f6fee9f` 가 떨어져 나갔다. 지금 그 커밋은
> 어느 ref 에서도 도달할 수 없는 orphan 이고, reflog 만 붙잡고 있다. 그 브랜치의
> PR #44 는 2026-09-11 에 그 상태로 squash 머지됐으므로, §2.1 은 `origin/main`
> 에 **한 번도 들어간 적이 없다**. (브랜치 자체는 지금도 살아 있고 origin 에도
> 푸시돼 있다 — 사라진 것은 브랜치가 아니라 커밋이다.)
>
> **2. PR #42 에 새 커밋으로 다시 반영했다.** 새 커밋은
> **`428a952` `docs(claude): ai-office 스택 예외 조항 재작성 — f6fee9f 유실분 대체`**
> 이다. `f6fee9f` 의 원문 28줄을 SHA 로 직접 꺼내 그대로 복원했으므로 내용은
> 원본과 동일하다. 이 커밋이 `main` 에 들어가면 §2.1 유실 위험은 사라진다.
>
> **3. kakao 세션 쪽에는 별도로 알리지 않았다.** 그 세션의 인수인계 문서
> (`SESSION-HANDOVER-2026-09-11-kakao-loop-matchposts.md`)는 `f6fee9f` 를 아직
> 자기 브랜치의 커밋으로 적어 두고 있다. 알리려 했으나 `ListAgents` 로 본 피어
> 세션이 대부분 offline 이고, 살아 있는 한 개도 그 세션인지 확인되지 않아
> 실행 불가로 판단했다. §2.1 은 위 `428a952` 로 이미 안전해졌으므로 그쪽의
> 미결 항목("`f6fee9f` 를 PR 에 둘지 뗄지")은 그냥 닫아도 된다.
