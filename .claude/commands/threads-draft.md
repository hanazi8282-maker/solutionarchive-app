...
역산 아카이브 1종 (남의 글에서 구조만 뽑은 것 — 위계 3층):

- `.claude/skills/content-gate/references/threads-playbook.md`
  먼저 볼 것: **§3-0 실측**(리스트형 좋아요율 16%대 vs 장문 연재 3~6%대,
  연재 3편까지 81% 이탈) · **§5 템플릿 5종과 500자 적합성** · **§8-4b 어미 기준** ·
  **§10 T-체크(TC-0~TC-20)**. TPL-B 는 사용하지 마라(§11-2).

기계가 쓴 파일 2종 (실측에서 나온 것 — 사람이 고치지 마라):

- `content/guides/learned-patterns.md` — 저장 글에서 추출돼 성과로 검증된 패턴 +
  발행자가 초안을 직접 고친 수정 패턴(키 `edit-`, 1건부터 실린다)
...
2. **`learned-patterns.md` 의 "기본값"·"권장" 패턴** — 실제 발행 성과로
   ...
   말 그대로 참고만 한다.
   패턴끼리 부딪히면 각 패턴의 **위계** 줄을 따른다. 정본은 `ops/roles/cmo.md` 의
   "콘텐츠 판단 위계"다(반응률 > 발행자 수정 `edit-` > 작가 기본값). Read 하고 따른다.
3. **사람이 쓴 가이드 3종** — 나머지 전부.

**voice-guide 는 위계 밖이 아니라 위계 2층이다** (2026-09-13 개정, E-1 해소).
이전 판은 "우선순위 밖"이라고 적었는데, 그래서 `cmo.md` 의 위계(발행자 수정본 = 2층)와
모순이었다. 지금 voice-guide 는 **발행자 리라이트를 성문화한 문서**이므로 2층이고,
작가 기본값(3층, `threads-playbook.md`)은 이것을 덮어쓰지 못한다.
반응률(1층)이 뒤집으면 **voice-guide 자체를 고친다** — 초안에서 몰래 벗어나지 않는다.

패턴은 "무엇을 어떤 구조로 말하는가"를 정하고, voice-guide 는 "누가 말하는가"를 정한다.

`learned-patterns.md` 가 "아직 반영된 패턴이 없다" 상태인 것은 정상이다 —
저장 글 근거 2건이나 발행자 수정 1건이 쌓이기 전까지는 비어 있다.
...
`voice-guide.md` §0 을 그대로 따른다. 두 모드가 있고, 고르는 기준은 취향이 아니라
**화자가 그 자리에 있었는가**다.

- **모드 A — 케이스 서술 (기본값, 실측 17/19)**
  남의 회사 무브를 옮길 때. **3인칭 · 평어체**(`~있었다 / ~택했다`) ·
  **평어체 질문 마무리**(`~단순한가? / ~있을까?`).
- **모드 B — 자체 경험 (2/19)**
  우리가 직접 겪은 일. **1인칭 '저/제' · 합쇼체**(`~습니다 / ~죠 / ~더군요`) ·
  해요체 질문 마무리(`~돌려보셨나요?`).

**섞지 마라.** 케이스 글에 "제가 보기엔"이 들어가는 순간, 공시에서 읽은 사실과
내 인상이 같은 문장에 놓여 독자가 둘을 못 가른다.

⚠️ 2026-09-13 이전 판은 "기본값은 합쇼체"라고 적었다. **틀린 지시였다** —
`drafts/threads/` 19건 중 합쇼체는 1건(자체 경험 소재)뿐이었다. 기억으로 쓰지 말고
최신 초안 2건을 실제로 열어 대조하라.

`seo-guide.md` 에서 가져오는 것은 톤이 아니라 **구조**다: 첫 줄 후킹, 500자 제한,
질문 마무리, 링크는 본문 아닌 자답, 주제 태그 1개.
(`seo-guide` 의 "반말·카톡체" 권장은 관찰된 문화 경향이지 플랫폼 규칙이 아니다.
음슴체 채택 여부는 N-0006 A/B 가 판정한다 — `threads-playbook.md` §11-4.)
...
- **모드를 먼저 정한다** (§2). 한 글 안에서 섞지 않는다.
- **모드 A 골격 4단**: 병목 제시 → **통상의 선택**(독자가 예상할 답) → 실제 무브 + 수치·귀속
  → 독자 질문. **2단이 빠지면 그냥 사례 소개가 된다. 반전은 2단이 만든다.**
- **모드 B 골격**: 과거의 나(무지·실패) → 전환 사건 → 깨달음 → 일반화 → 독자 질문.
  실패를 먼저 고백하고 신뢰를 산다.
- 두 모드 공통: 가르치지 않는다. 자랑하지 않는다. 마지막 한 줄은 질문으로 끊는다(19/19).
- 한 문장 = 한 단락으로 핵심을 독립시킨다. 단락은 2~4문장, 사이에 빈 줄.
- 장단 교차: 긴 설명 뒤 한 줄 단정.
- ❌ 본문에 기호(`→` `—` `+6.6%`) 금지. em-dash 로 반전 만들지 않기. 경구로 압축하지 않기.
- ❌ 느낌표 남발 금지. 정의·배경으로 시작 금지. 마무리 장황한 요약 금지.
