-- 롤백: brunch · clien · fmkorea 커뮤니티 소스 kill-switch
...
UPDATE public.review_sources
   SET enabled = false, disabled_reason = '롤백: 커뮤니티 소스 round-3(brunch·clien·fmkorea) 철회'
 WHERE key IN ('brunch', 'clien', 'fmkorea');

-- 행 자체를 지우려면 review_targets / review_fingerprints / analysis_inputs 의
-- source_key IN ('brunch','clien','fmkorea') 행을 먼저 정리해야 한다(FK).
-- 그건 데이터 삭제라 사람이 판단한다.
--
