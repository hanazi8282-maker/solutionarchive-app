...
export function proposalPrompt(kind, count, known) {
  ...
  return [
    ...
    '',
    '조건:',
    // ⚠️ 이 줄의 방향은 게이트(lib/discovery/candidate.ts MAX_VOC_HITS)와 **같아야 한다.**
    //    2026-09-18 까지 여기 "**많이** 쌓였을 법한 것" 이라고 적혀 있었다. 상한 500 이
    //    들어간 뒤로 프롬프트는 볼륨 최대화를 시키고 게이트는 초과분을 기각해서, 첫 자동
    //    실행이 Notion 79,077 · Datadog 3,576 을 내고 **채택 0건**으로 끝났다.
    //    창(30~500)이 비어서가 아니다 — 같은 날 HN 실측으로 Typesense 475 · Cal.com 320 ·
    //    Chatwoot 115 · Papermark 34 처럼 창 안에 사는 제품이 얼마든지 있었다. 표본을 뽑는
    //    방향만 반대였다.
    //
    // ⚠️ 그래도 **채택 창의 숫자를 프롬프트에 알려주지 않는다.** 알려주면 모델이 hits 를
    //    추정해 스스로 걸러내기 시작하고, 그건 아래 "추측해서 쓰지 마라"와 정면으로
    //    어긋난다 — 같은 종류의 모순이다. 프롬프트는 **표본 분포만** 민다. 채택은 프로브가
    //    센 숫자만 정한다.
    `- ${source} 에 실제 후기·토론이 쌓여 있긴 하지만, **누구나 아는 대형 브랜드는 아닌** 것.`,
    '- 규모가 애매하면 **작은 쪽을 골라라.** 후기가 너무 많은 것도 이 도구가 기각한다 — 우리 독자가 따라 할 수 없는 규모라서다.',
