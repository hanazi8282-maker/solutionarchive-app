...
- **push·PR — 첫 시도 음성, 리포 추가 후 양성.** 세션이 리포를 소스로 붙이지 않은 채 시작돼(그래서 `origin` 도 없었다) 첫 `git push` 는 git 프록시에서 403 이었다("not in this session's authorized repository set"). 같은 세션에서 `add_repo`(push 권한)로 리포를 붙인 뒤 push 가 성공했고 GitHub API 도 200 으로 바뀌었다. `gh` CLI 는 미설치라 PR 은 `GH_TOKEN`(값 미기재)으로 GitHub REST API 를 직접 호출해 만들었다. **교훈: `claude --cloud` 세션은 리포가 자동으로 붙지 않을 수 있다 — 첫 단계에서 `origin`·push 권한을 확인할 것.**
